...
#   eigen - Eigen's bundled kissfft-backed FFT (no external deps;
#           the choice for cross-compiled/Nerves targets)
...
ifneq ($(NX_EIGEN_FFT_SO),)
  ...
  # Eigen's bundled kissfft-backed FFT (unsupported/Eigen/FFT):
  # header-only, no external library — the choice for cross-compiled
  # targets without FFTW.
